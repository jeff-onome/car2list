          ...
          {rentals.length > 0 ? rentals.map(r => {
            ...
            return (
              <div 
                ...
                className={`glass p-6 rounded-[1.5rem] border transition-all duration-500 cursor-pointer active:scale-[0.99] ${isExpanded ? 'bg-white/[0.05] border-white/20' : 'bg-white/[0.02] border-white/5 hover:border-white/10'}`}
              >
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                  <div className="flex gap-6 items-center w-full sm:w-auto">
                    <div className="w-12 h-12 rounded-full bg-zinc-900 flex items-center justify-center border border-white/5 shrink-0 text-zinc-500">
                    ...
                    <div className="overflow-hidden">
                       ...
                       <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-[9px] text-zinc-500 font-bold uppercase truncate">{r.userName}</span>
                       </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-10 w-full sm:w-auto border-t sm:border-t-0 border-white/5 pt-4 sm:pt-0">
                     ...
                     <div className="flex items-center gap-4">
                        <Link 
                          ...
                          className="bg-white text-black px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-zinc-200 transition-all shadow-lg"
                        >
                          Manage
                        </Link>
                        <div className={`transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}>
                           <svg className="w-5 h-5 text-zinc-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
                        </div>
                     </div>
                  </div>
                </div>
                ...
                {isExpanded && (
                  <div className="mt-8 pt-8 border-t border-white/5 grid grid-cols-1 md:grid-cols-3 gap-8 animate-in fade-in slide-in-from-top-4 duration-500">
                     <div className="space-y-4">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Identity Record</p>
                        <div className="space-y-1">
                           ...
                           <p className="text-[8px] text-zinc-600 uppercase font-bold mt-2">Internal ID: {r.id}</p>
                        </div>
                     </div>
                     <div className="space-y-4">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Logistics Detail</p>
                        <div className="space-y-2">
                           <div className="flex justify-between text-[10px]">
                              <span className="text-zinc-600 uppercase">Lease Term</span>
                              <span className="text-white">{r.duration} Days</span>
                           </div>
                           <div className="flex justify-between text-[10px]">
                              <span className="text-zinc-600 uppercase">Drop Point</span>
                              <span className="text-white truncate max-w-[120px]">{r.location || 'Private Port'}</span>
                           </div>
                        </div>
                     </div>
                     <div className="space-y-4">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Financial Telemetry</p>
                        <div className="space-y-2">
                           <div className="flex justify-between text-[10px]">
                              <span className="text-zinc-600 uppercase">Total Valuation</span>
                              <span className="text-white font-bold">{formatPrice(r.totalPrice)}</span>
                           </div>
                           <div className="flex justify-between text-[10px]">
                              <span className="text-zinc-600 uppercase">Channel</span>
                              <span className="text-zinc-400">Verified Member Ledger</span>
                           </div>
                        </div>
                     </div>
                  </div>
